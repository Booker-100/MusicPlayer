...
import static ml.yike.yueyin.database.DatabaseHelper.ID_COLUMN;
import static ml.yike.yueyin.database.DatabaseHelper.MUSIC_ID_COLUMN;
...
public class DBManager {
    ...
    public MusicInfo getSingleMusicFromMusicTable(int id) {
        ...
        try {
            ...
            musicInfoList = cursorToMusicList(cursor);
            musicInfo = musicInfoList.get(0);  //id是专属的
            db.setTransactionSuccessful();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public List<PlayListInfo> getMyPlayList() {
        ...
        while (cursor.moveToNext()) {
            ...
            cursorCount = db.query(DatabaseHelper.PLAY_LISY_MUSIC_TABLE,null, ID_COLUMN + " = ?", new String[]{"" + id}, null,null,null);  //获取歌单里面歌曲的数量
    ...
    public void insertMusicInfoToMusicTable(MusicInfo musicInfo) {
        ContentValues values;  //存储到values中
        Cursor cursor = null;
        int id = 1;
        try {
            ...
            if (cursor.moveToFirst()) {
                id = cursor.getInt(0) + 1;   //向后添加，设置新添加的ID为最大ID+1
            }
            values.put(ID_COLUMN, id);
            db.insert(DatabaseHelper.MUSIC_TABLE, null, values);  //更新music表
        } catch (Exception e) {
            e.printStackTrace();
            if (cursor!=null){
                cursor.close();
    ...
    public int getPreMusic(ArrayList<Integer> musicList, int id, int playMode) {
        ...
        int index = musicList.indexOf(id);  //找到当前id在列表的第几个位置（i+1）
        if (index == -1) {
            return -1;
        }
        switch (playMode) {
    ...
    public ArrayList<String> getMusicInfo(int id) {
        ...
        Cursor cursor = null;
        ArrayList<String> musicInfos = new ArrayList<String>();
        cursor = db.query(DatabaseHelper.MUSIC_TABLE, null, ID_COLUMN + " = ?", new String[]{"" + id}, null, null, null);
        if (cursor.moveToFirst()) {
            for (int i = 0; i < cursor.getColumnCount(); i++) {
                musicInfos.add(i, cursor.getString(i));
            }
        } else {
            musicInfos.add("0");
            musicInfos.add("听听音乐");
